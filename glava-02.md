...
Asterisk построен на модулях. Модул ь -это загружаемый компонент, который предоставляет определенную функциональность, такую как драйвер канала \(например, _chan\_pjsip.so_\), или ресурс, разрешающий подключение к внешней технологии \(например _func\_odbc.so_\). Модули Asterisk загружаются на основе параметров, определенных в файле _/etc/asterisk/modules.conf_. Мы обсудим использование многих модулей в этой книге, но на данном этапе мы просто хотим ввести понятие модулей и дать вам представление о типах доступных модулей.

На самом деле можно запустить Asterisk вообще без каких-либо модулей, хотя в этом состоянии он ничего не сможет сделать. Это полезно, чтобы понять модульную природу  Asterisk, чтобы оценить архитектуру.

{% hint style="info" %}
**Примечание**

Вы можете запустить Asterisk без модулей, загруженных по умолчанию и загружать каждый нужный модуль вручную из консоли, но это не то, что вы хотели бы запускать в продакшене; это было бы полезно только в том случае, если бы вы настраивали производительность системы, где необходимо убрать лишнее, что не требуется вашим конкретным приложениям Asterisk.
{% endhint %}
