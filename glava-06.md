...
Контексты определяются в файле _extensions.conf._ Имя контекста помещается в квадратные скобки \(\[ \]\). Имя может состоять из букв A - Z \(верхний и нижний регистр\), чисел от 0 до 9, а также дефиса и подчеркивания. Контекст для входящих вызовов от оператора связи может быть назван так:
...
{% hint style="info" %}
Обратите внимание, что пробел явно отсутствует в списке разрешенных символов. Не используйте пробелы в именах контекстов — вам не понравится результат!
{% endhint %}
